...
def build_pptx(filename="HWP_attack_analysis_fixed.pptx"):
    prs = Presentation()
    ...
    p = tf.add_paragraph()
    p.text = "취약점: CVE‑2017‑8291 (EPS PostScript 쉘코드 실행)"
    p.level = 1
    ...
    p = tf.add_paragraph()
    p.text = "GetProcAddress → system 호출 → VBScript 실행"
    p.level = 1
    ...
    tf = slide.placeholders[1].text_frame
    plans = ["Ghostscript/HWP 최신 패치 적용",
             "regsvr32 실행 제한 및 C2 URL 차단",
             "정기적 보안 교육 및 모의 훈련"]
    tf.text = plans[0]
    for txt in plans[1:]:
        p = tf.add_paragraph()
